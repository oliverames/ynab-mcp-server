...
**Left off at**: Pushed v1.6.0. Like v1.5.0, the npm package still needs `./publish.sh` to bump from 1.4.0 (package.json is behind index.js). Connector source updated; installed cache and npm publish are pending. Skill side: companion `ynab-finance` skill in ames-claude got a new "Transaction Approval Workflow" section with a flags reference table and explicit batch-approval gate rules.
